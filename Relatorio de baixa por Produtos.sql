...
SELECT
	...
	E1_EXT.MULTA, 
	CASE
		WHEN E1_EXT.ATRASO > 0 THEN TO_CHAR(E1_EXT.ATRASO) ELSE '' 
	END AS ATRASO, 
	E1_EXT.DEMISSAO, 
	E1_EXT.DVENCTO, 
	E1_EXT.WEMISSAO, 
	...
	E1_EXT.VEND_GER, 
	CASE
		WHEN E1_EXT.ATRASO > 0 THEN E1_EXT.SALDO ELSE 0 
	END AS VL_VENCIDO, 
	CASE
		WHEN E1_EXT.ATRASO > 0 
		THEN ROUND(E1_EXT.SALDO * (1 + (E1_EXT.TX_JUROS * E1_EXT.ATRASO + E1_EXT.MULTA) / 100) * (1 - nDesconto / 100), 2) 
		ELSE ROUND(E1_EXT.SALDO * (1 - nAntecipcao / 100) * (1 - nDesconto / 100), 2) 
	END AS VL_CORRIG, 
	CASE
		WHEN E1_EXT.ATRASO > 0 THEN ROUND(E1_EXT.SALDO * ((E1_EXT.TX_JUROS * E1_EXT.ATRASO + E1_EXT.MULTA) / 100), 2) ELSE 0 
	END AS VL_JUROS 
FROM
	(
	SELECT 
		...
		E1.E1_EMISSAO AS DEMISSAO, 
		E1.E1_VENCREA AS DVENCTO, 
	...
	WHERE 
		E1.E1_TIPO = 'NF' 
		AND E1.E1_FILORIG IN ('FG01') 
		AND A1.R_E_C_N_O_ IN (18701,15096,18190,15628,15669,14533,14542,15620,15706,15123)
		AND E1.E1_SITUACA IN ('0', '1', '2', '3', '4', '5', '6', '7', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O') 
		AND E1.E1_VENCTO BETWEEN '20251001' AND '20251031' 
		AND E1.E1_EMISSAO BETWEEN '20250810' AND '20250816' 
...
SELECT 
	...
	ROUND('E1_VALOR' * D2.D2_TOTAL / F2_VALBRUT, 2) AS PROPORCIONAL 
FROM 
	SD2010 D2 
	INNER JOIN 
		SF2010 F2 
...
-- Clientes mais usados

SELECT 
	D_E_L_E_T_ AS DEL, 
	R_E_C_N_O_ AS REC, 
	A1_FILIAL, 
	A1_COD || '-' || A1_LOJA AS ID 
FROM 
	SA1010 
WHERE 
	A1_COD || '-' || A1_LOJA IN ('003724687-0001', '084500310-0001', '000516169-0001', '004142810-0001', '103333160-0001', '445932640-0001', '220554560-0001', '204771690-0001', '005189690-0001', '042827500-0001') 
	AND D_E_L_E_T_ <> '*' 
ORDER BY ID 
